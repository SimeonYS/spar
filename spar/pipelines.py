...
class SparPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `spar`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM spar WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO spar (date, title, link, content)"
